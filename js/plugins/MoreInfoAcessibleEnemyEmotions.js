!function() {
  Scene_Battle.prototype.addActorActionPromptLogText = function() {
    ...
    // If Actor Exists
    if (actor) {
	  	let weapon = actor.weapons()[0] === undefined ? '-----' : actor.weapons()[0].name
	let charm = actor.armors()[0] === undefined ? '-----' : actor.weapons()[0].name
      this.addLogCommandMessage(LanguageManager.languageData().text.XX_GENERAL.message_104.text.format(actor.name()));
      this.addLogCommandMessage("[\\fs[16]ATK: "+actor.atk+", DEF: "+actor.def+", SPD: "+actor.agi+", LCK: "+actor.luk+", HIT: "+Math.round(actor.hit * 100)+"\\fs[24]]", false);
	  this.addLogCommandMessage("[\\fs[16]WEAPON: "+weapon+", CHARM: "+charm+"\\fs[24]]", false);
    };
  };
