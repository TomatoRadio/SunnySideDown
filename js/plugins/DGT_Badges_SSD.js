...
//  - improve language support (thanks aoisensei)
{
...

    {
        function main() {
            ...
            DGT.UnlockBadge = function (modId, badgeName) {
			...
			DGT.SilentUnlock = function (modId, badgeName) {
                let modData = DGT.Badges._data[modId]
                if (!modData || !modData[badgeName]) {
                    DGT.BadgeError(modId, badgeName)
                    return null
            ...
            DGT.startBadgeNotification = function (modId, badgeName) {
                let meta = DGT.Badges._metadata[modId]
                let data = DGT.Badges._data[modId][badgeName]
                
                let border = data.custom_toast || meta.badge_toast_border || 'badge_toast_border'
                let img = data.img
                ...
                DGT.makeBadgeToast(corner, border, img, ox, oy)
            }
            DGT.makeBadgeToast = function (corner, border, img, ox, oy) {
                let newToast = new DGT.BadgeToast(corner, border, img, ox, oy)
                SceneManager._scene.addChild(newToast)
            }
            ...
            class BadgeInfoWindow extends Window_Base {
                ...
                drawProgress(mod, meta) {
                    ...
                    this.drawText(`BADGE COMPLETION: ${unlocked} of ${total} (%${percent})`, 13, 87)
                    // width of bar: 446
                    // height: 12
                    let [bx, by] = [13, 117]
                    ...
                    if (unlocked === total && !(total === 0)) {
                        this.contents.gradientFillRect(bx + 2, by + 2, barWidth, 4, '#FFAEFF', '#FFFF88')
                    } else {
                    ...
                    if (modunlocked === modtotal && !(modtotal === 0)) {
                        this.contents.gradientFillRect(bx + 2, by + 6, modBarWidth, 4, '#FFAEFF', '#FFFF88')
                    } else {
            ...
            class BadgeScene extends Scene_Base {
                ...
                flattenBadgeData() {
                    ...
                    for (let [modId, modData] of Object.entries(DGT.Badges._data)) {
                        for (let [badgeId, badgeData] of Object.entries(modData)) {
                            this._data.push({ id: badgeId, modId, data: badgeData, meta: DGT.Badges._metadata[modId], unlocked: DGT.isBadgeUnlocked(modId, badgeId) })
                        }
                    }
                }
                createBadgeWindow() {
                ...
                select(index) {
                    ...
                    this._badgeInfoWindow.updateText(name, description, data.modId, data.meta)
                }
                update() {
                    super.update()
            ...
            class BadgeToast extends Window_Base {
                constructor(corner, border, img, imgx, imgy) { super(corner, border, img, imgx, imgy); }
                initialize(corner, border, img, imgx, imgy) {
                    super.initialize(0, 0, this.windowWidth(), this.windowHeight())
                    this.setParameters()
                ...
                update() {
                    if (this._currentPhase === 'done') { this.destroy(); return }
    ...
    loadReservedBitmaps = function () { //stupid
        ...
        for (let [modId, modData] of Object.entries(DGT.Badges._data)) {
            ...
            if (meta.other_toast_borders) {

                for (var i = 0; i<meta.other_toast_borders.length; i++){
                    DGT.__aaaaa.push(ImageManager.loadSystem(meta.other_toast_borders[i], 0))
                }
